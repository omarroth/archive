...
const errorCodes = require("./util/codes.js");
			...
			new Promise(resolve => {
				...
				if (req.method == "POST" || req.method == "PATCH") {
					...
					req.on("end", (chunk) => {
						...
						if (h.worker) {
							const extra = require("./util/extra.js")({db: sqlite});
							extra.getWorker(params).then(worker => {
								if (typeof(worker) != "string") resolve(worker);
								else h.code({req, reqPath, res, fill, params, worker, body, data}).then(resolve);
							});
						} else {
					...
					if (h.worker) {
						...
						extra.getWorker(params).then(worker => {
							if (typeof(worker) != "string") resolve(worker);
							else h.code({req, reqPath, res, fill, params, worker}).then(resolve);
						});
					} else {
				...
				if (result.content && result.content.status == "error" && result.content.code) {
					result.content.message = errorCodes[result.content.code];
				}
